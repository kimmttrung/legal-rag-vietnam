...
## 2. Thêm YAML front-matter cho Space
Space đọc cấu hình từ `README.md`. Vào tab **Files** của Space, sửa `README.md` để **bắt đầu** bằng:
...
(Phần dưới front-matter viết gì cũng được.)

## 3. Đưa code lên Space
Space là một git repo riêng. Từ máy bạn:
...
| `LLM_API_BASE_URL` | vd `https://api.deepinfra.com/v1/openai` |
| `LLM_API_KEY` | API key LLM |
| `LLM_API_MODEL` | vd `Qwen/Qwen2.5-7B-Instruct` |
